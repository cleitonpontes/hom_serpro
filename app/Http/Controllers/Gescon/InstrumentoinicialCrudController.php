...
//                'prefix' => "R$",
    ...
    public function store(StoreRequest $request)
    {
        ...
        $request->request->set('valor_parcela', number_format((float)$valor_parcela, 2, '.', ''));

        $valor_global = str_replace(',', '.', str_replace('.', '', $request->input('valor_global')));
        $request->request->set('valor_global', number_format((float)$valor_global, 2, '.', ''));
        $request->request->set('valor_inicial', number_format((float)$valor_global, 2, '.', ''));
